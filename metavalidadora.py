# METACLASE utilizando 'type' para validar la estructura de las clases 
class MetaValidadora(type):  # Definición de la metaclase heredando de 'type'
    def __new__(cls, name, bases, dct):  # Método que se ejecuta al crear una nueva clase
        # Verifica si la clase no es la base y si le falta el método polimórfico 
...
# DECORADOR PROPIO: Valida la disponibilidad del libro antes de prestarlo
